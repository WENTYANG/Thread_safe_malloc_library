...
int main(int argc, char *argv[])
{
  ...
  struct timespec malloc_time0, malloc_time1, free_time0, free_time1;
  double myfreetime = 0;
  ...
  for (i=0; i < NUM_ITERS; i++) {
    ...
    for (j=0; j < NUM_ITEMS; j+=50) {
      ...
      clock_gettime(CLOCK_MONOTONIC, &free_time0);
	...
	FREE(malloc_items[malloc_set][item_to_free].address);
      ...
      clock_gettime(CLOCK_MONOTONIC, &free_time1);

      clock_gettime(CLOCK_MONOTONIC, &malloc_time0);

      for (k=0; k < 50; k++) {
	      malloc_items[1-malloc_set][j+k].address = (int *)MALLOC(malloc_items[1-malloc_set][j+k].bytes);
      } //for k

      clock_gettime(CLOCK_MONOTONIC, &malloc_time1);

    myfreetime += calc_time(free_time0, free_time1);
    mymalloctime += calc_time(malloc_time0, malloc_time1);
    } //for j
  } //for i
